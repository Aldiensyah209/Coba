...
<div class="card w-100">
  <div class="card-body p-4">
    ...
    <div class="d-flex align-items-center justify-content-between mb-9">
      ...
      <a href="#" class="btn btn-primary fw-semibold" data-bs-toggle="modal" data-bs-target="#tambahBajuModal"><i class="ti ti-plus fw-semibold me-2"></i>Tambah Baju</a>
    </div>
    <!-- Data Table -->
      ...
      <table class="table mb-4 align-middle">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td class="border-bottom-0">
              <h6 class="fw-normal mb-0">{{ $item->nama_baju }}</h6>
            </td>
            <td class="border-bottom-0">
              <h6 class="fw-normal mb-0">{{ "Rp " . number_format($item->harga,2,',','.') }}</h6>
            </td>
            <td class="border-bottom-0">
              @if ($item->gambar_baju)
              <img src="{{ asset('images/post/' . $item->gambar_baju) }}" alt="{{ $item->gambar_baju }}" width="100">
              @else
              Tidak ada gambar
              @endif
            </td>
            ...
            <td class="border-bottom-0">
              ...
              <form action="{{ route('baju.destroy', $item->id) }}" method="POST">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn btn-danger m-1" onclick="return confirm('Apakah Anda yakin ingin menghapus baju ini?')">
...
<div class="modal fade" id="tambahBajuModal" tabindex="-1" aria-labelledby="tambahBajuModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <form id="createForm" action="{{ route('baju.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('POST')
        <div class="modal-body">
          <div class="mb-3">
            <label for="nama_baju" class="form-label">Nama Baju</label>
            <input type="text" class="form-control" id="nama_baju" name="nama_baju" oninvalid="this.setCustomValidity('Bidang tidak boleh kosong')" oninput="setCustomValidity('')" required>
          </div>
          <div class="mb-3">
          ...
          <div class="mb-3">
            ...
            <input type="file" class="form-control" id="gambar_baju" name="gambar_baju" oninvalid="this.setCustomValidity('Bidang tidak boleh kosong')" oninput="setCustomValidity('')" required>
          </div>
          <div class="mb-3">
      ...
      <form id="updateForm" method="POST" enctype="multipart/form-data">
        ...
        <div class="modal-body">
          <div class="mb-3">
            ...
            <input type="text" class="form-control" id="edit_nama_baju" name="edit_nama_baju" required>
          </div>
          <div class="mb-3">
